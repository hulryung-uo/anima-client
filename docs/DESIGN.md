...
└── crates/
    ...
    │       │   ├── game.rs              # game packet codec → World mutation (66 incoming ids, §8)
...
5. ✅ Game packet codec → World mutation (`net/game.rs`): originally 0x20/0x77/0x78/
   0x1A/0x1D/0x11/0xA1-3/0x1C/0xAE/0xBF; now **66 incoming ids** dispatched (count
...
- **Game-phase incoming coverage (current, verified by counting `net::game::dispatch`'s
  match arms):** **66** packet ids handled in `net/game.rs` — `0x20` MobileUpdate,
  ...
  `0x2D` full mobile attributes, `0x89` corpse equip, `0xC2`
  ...
  for **68** total. Outgoing login-phase `0x83` delete-character is handled
