...
| Accounts and servers | Multiple profiles, optional OS passwords, restart persistence, cache accuracy | Library, browser UI and macOS vault verified. Password changes now stage file writes first and undo reported vault/rename failures; six additional regressions cover lost-credential cases, rollback failure and oversized caches. An actual macOS Keychain/profile lifecycle check passed. Windows OS-vault runtime and profile repair/export remain. |
