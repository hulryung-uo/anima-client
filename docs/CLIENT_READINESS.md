...
| --- | --- | --- |
| First launch and game files | Clean launch, discover/pick/change valid files, actionable missing-file errors | Auto-detection and picker exist; failure recovery and changing files need review. |
...
| Settings and state | Reliable persistence, recovery from corrupt data, useful backup/restore, isolation across characters/windows | Profile storage is guarded; desktop configuration and renderer preference recovery need audit. |
| Reliability and performance | No stale callbacks, clear disconnect/crash behavior, bounded resources, responsive long sessions | Connection races and unbounded login waits repaired; 2026-09-13 gate passed (654 Rust tests, 249 web tests). Long-session, asset-cache and process-crash audit remains. |
